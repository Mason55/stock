...
class RealtimeDataFetcher:
    ...
    async def fetch_sina_realtime(self, stock_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Unexpected error fetching Sina data for {stock_code}: {e}", exc_info=True)
    ...
    def _parse_sina_response(self, text: str, stock_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            if len(parts) < SINA_MIN_RESPONSE_FIELDS:
                logger.warning(f"Insufficient fields in Sina response for {stock_code}: {len(parts)}")
                return None
...
class HistoricalDataFetcher:
    ...
    async def _try_yahoo(self, stock_code: str, days: int) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            data = yf.download(yf_symbol, period=f"{period_days}d", interval='1d',
...
class TechnicalIndicatorCalculator:
    ...
    @staticmethod
    def calculate_macd(series: pd.Series, fast: int = 12, slow: int = 26, signal: int = 9) -> Dict[str, Optional[float]]:
